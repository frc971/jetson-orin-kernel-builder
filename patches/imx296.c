...
static int imx296_read(struct camera_common_data *s_data, u32 addr)
{
...

	{
		...
		dev_dbg(s_data->dev, "%s, regmap_raw_read(): addr=%x, data[2]=%x, data[1]=%x, data[0]=%x\n", __func__, addr, data[2], data[1], data[0]);

		result1 = (data[2] << 16) | (data[1] << 8) | data[0];
		//result1 = ((data[2] & 0xff) << 16) | ((data[1] & 0xff) << 8) | (data[0] & 0xff);
	}
...
/*
	{
		...
		if (bytes <= 1) {
			...
			ret = regmap_read(s_data->regmap,
					  ...
					  //&(data[bytes - 1]));
					  &reg_val);
			dev_dbg(s_data->dev, "regmap_read(): bytes-1=%d, reg_val=%x\n", bytes-1, reg_val);
			data[bytes - 1] = reg_val & 0xff;
			dev_dbg(s_data->dev, "regmap_read(): bytes-1=%d, data[0,1,2]=%x,%x,%x\n", bytes-1, data[0], data[1], data[2]);
			if (ret < 0)
				return ret;
		}
		if (bytes <= 2) {
			...
			ret = regmap_read(s_data->regmap,
					  ...
					  //&(data[bytes - 2]));
					  &reg_val);
			dev_dbg(s_data->dev, "regmap_read(): bytes-2=%d, reg_val=%x\n", bytes-2, reg_val);
			data[bytes - 2] = reg_val & 0xff;
			dev_dbg(s_data->dev, "regmap_read(): bytes-2=%d, data[0,1,2]=%x,%x,%x\n", bytes-2, data[0], data[1], data[2]);
			if (ret < 0)
				return ret;
		}
		if (bytes <= 3) {
			...
			ret = regmap_read(s_data->regmap,
					  ...
					  //&(data[bytes - 3]));
					  &reg_val);
			dev_dbg(s_data->dev, "regmap_read(): bytes-3=%d, reg_val=%x\n", bytes-3, reg_val);
			data[bytes - 3] = reg_val & 0xff;
			dev_dbg(s_data->dev, "regmap_read(): bytes-3=%d, data[0,1,2]=%x,%x,%x\n", bytes-3, data[0], data[1], data[2]);
			if (ret < 0)
				return ret;
		}
			...
			int *err)
{
	...
	int ret = 0; //<<JC>> was int ret;

	//dev_dbg(s_data->dev, "%s, imx296_write(): addr=%x, data[2]=%x, data[1]=%x, data[0]=%x\n", __func__, addr, data[2], data[1], data[0]);

	if (err && *err) {
		dev_err(s_data->dev, "err or *err occurred!\n");
		return *err;
	}
...
static int imx296_set_mode(struct tegracam_device *tc_dev)
{
	...
	// JC: 20250503
	imx296_write(s_data, IMX296_GAIN, 0x0200, &err);
	imx296_write(s_data, IMX296_SHS1, 0x0800, &err);
