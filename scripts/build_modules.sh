...
build_module() {
    ...
    if [ $is_external -eq 0 ]; then
        ...
        if [ -n "$ko_file" ] && [ -f "$ko_file" ]; then
            ...
            MODULES_DICT["$module_file"]="${module_path}"
        else
        ...
        for ko_file in "$STAGING_DIR"/*.ko; do
            if [ -f "$ko_file" ]; then
                local module_file=$(basename "$ko_file")
                MODULES_DICT["$module_file"]="$module_path$module_file"
...
install_modules() {
    for module_file in "$STAGING_DIR"/*.ko; do
        if [ -f "$module_file" ]; then
            ...
            local target_subdir=$(dirname "${MODULES_DICT[$module_name]}")
            local target_dir="${INSTALL_DIRECTORY}/${target_subdir#${KERNEL_URI}/}"
...
prompt_install() {
    ...
    if [[ "$response" =~ ^[Yy]$ ]]; then
        ...
        echo "  sudo cp -v $STAGING_DIR/*.ko $INSTALL_DIRECTORY/<appropriate_subdir>"
        echo "  sudo depmod -a"
